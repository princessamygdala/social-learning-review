...
- **learning_mode_description:** - Learning mode: Learning from opponent's competitive/cooperative choices about opponent's competitiveness to guide own strategic behavior   - Learning from:     - Source type (social): other (opponent)     - Source content (social): action/policy (competitive or cooperative choice)   - Learning about:     - Target type (social): other (opponent)     - Target content (social): state (behavioral tendency / competitiveness level)
- **task_description:** Participants played a repeated Chicken Game against two simulated opponents (one highly competitive at 70% straight, one less competitive at 30% straight) over 200 trials across 5 blocks. On each trial, participants chose to "go straight" (compete) or "turn" (cooperate), receiving payoffs contingent on both players' choices.
- **task_paradigm:** Coordination game
- **players:** Single agent (participant), multi-target (2 simulated opponents: 1 highly competitive, 1 less competitive)
- **n_players:** multi-target (3+)
- **partner_type:** computer (algorithmic)
- **stimuli:** Human face photographs (neutral expression), car icons, numerical point feedback
- **method:** behavioural
- **method_full:** behavioural
- **main_result:** - Significant main effect of Competition on competition rate (F(1,28) = 43.54, ηp² = 0.609, BF10 = 2.748 x 10⁸) - Significant Competition x Block interaction (F(4,112) = 8.64, ηp² = 0.229, BF10 = 36410.563) - Progressive decrease in competitiveness against HP across blocks (Block 1 vs. Block 5: d = 1.103) - Stable competitiveness against LP across blocks - Model_1B best fitting model (vs. Model_1P: d = -0.621; vs. Model_1BP: d = -2.312) - Higher learning rate for cooperative than competitive feedback (d = -0.430)
- **effect_size:** ηp² = 0.609 (Competition main effect); ηp² = 0.229 (Competition x Block); d = 1.103 (Block 1 vs. Block 5 against HP); d = -0.430 (αB2 > αB1)
- **learning_from:** Other (opponent); opponent's competitive/cooperative choice each trial
- **learning_about:** Other (opponent); opponent's competitiveness level / behavioral tendency
- **outcome_modality:** unclear

## Algorithmic level
- **winning_model:** Model_1B: Rescorla-Wagner with 2 learning rates for opponent behavior (αB1 for competition feedback, αB2 for cooperation feedback; 1 β). P(t+1) = P(t) + αB * PE(t); softmax choice rule.
