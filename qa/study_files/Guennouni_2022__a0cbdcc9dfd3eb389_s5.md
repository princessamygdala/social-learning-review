...
- **study_id:** `a0cbdcc9dfd3eb389_s5`
- **on_website:** NO — DROPPED (verify below)
- **why_not_on_website:** FLAG: No human behavioral data collected. Purely computational simulation — POMDP trustee agents play against HMM investors. Does not meet inclusion criterion of "uses human behavioral data." However, the POMDP framework and utility function parameters were calibrated against human data from Chapter 3.

> Check each field below against the paper. Log errors in `field_verification.csv`.

## Identification
- **citation:** Guennouni, I. (2022). Strategic inference in social interaction: An experimental and computational account [Doctoral dissertation, University College London]. Chapter 6.
- **doi:** Not available
...
## Computational level
- **study_focus:** POMDP simulation of optimal trustee policy; determinants of cooperation emergence (depth of planning, social preferences, opponent model accuracy)
- **study_focus_short:** POMDP simulation of optimal trustee policy
...
## Algorithmic level
- **winning_model:** POMDP trustee with 3-state dynamic utility function (Fehr-Schmidt inequity aversion with state-dependent parameters) + 3-state HMM investor. Best match to human data: d=4 depth of planning with dynamic utility (Euclidean distance = 4.50 vs 16.53 for static).
...
- **eligibility_flag:** FLAG: No human behavioral data collected. Purely computational simulation — POMDP trustee agents play against HMM investors. Does not meet inclusion criterion of "uses human behavioral data." However, the POMDP framework and utility function parameters were calibrated against human data from Chapter 3.
...
- **other_notes:** Key theoretical finding: pro-social preferences with moderate planning depth (d=4) produces behavior closest to human data, functionally equivalent to high planning depth (d≥6) with purely selfish preferences. This suggests pro-social preferences may be a computational shortcut for sophisticated planning.  ---  ## General Discussion / Limitations (Chapter 7)
