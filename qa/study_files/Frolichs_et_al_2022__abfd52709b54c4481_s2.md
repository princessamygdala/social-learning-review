...
- **eligibility_flag:** Borderline -- uses RSA (representational similarity analysis) not computational models of learning. The RSA models test whether knowledge structures are represented in neural activity but do not model learning dynamics. Study 2 sub-study 2 has no feedback phase (no trial-by-trial learning). Sub-study 1 is a reanalysis of Korn et al. (2012). Flagged as borderline for "uses computational modeling" and "learning happens over time" criteria.
