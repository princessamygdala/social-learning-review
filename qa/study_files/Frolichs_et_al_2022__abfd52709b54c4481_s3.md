...
## Algorithmic level
- **winning_model:** HIGH -- varies across experiments, clearly reported
...
- **contrast:** 
- **key_regions:** 
- **coordinates_peak:** HIGH -- dmPFC cluster at 6, 47, 28 reported in Table 4.1  ## Cannot find - Fitted mean parameter values for winning models (alpha, gamma) -- not reported in dissertation text for individual experiments - Open data/code sharing statement for raw data (code is on GitHub but data availability unclear for dissertation) - DOI for the dissertation itself  ## Other notes - This dissertation explicitly uses Marr's tri-level framework to structure its studies, making it directly relevant to the review's framing - Study 1 is the primary qualifying study; it introduces a novel framework of adding "knowledge structures" (granularity + reference points) to standard Rescorla-Wagner models for social learning - The key innovation is modeling how people flexibly switch between coarse (Big-5 factor level) and fine (item-level similarity) representations depending on information availability - The stereotype reference point finding (Exp 4) has implications for understanding how prior social knowledge/stereotypes influence personality learning - The published version (Frolichs et al., 2022, Nat Comms) should be checked for potential duplicate in the corpus
...
- **eligibility_flag:** MEDIUM -- borderline cases requiring judgment call
