...
## Computational level
- **study_focus:** Testing biased sampling alternative account -- full vs partial feedback on observational bias transmission
- **study_focus_short:** Testing biased sampling alternative account -- full vs partial feedback on
...
- **task_description:** Same paradigm but participants were assigned to either partial feedback (chosen player only, as in Studies 1-3) or full feedback (both chosen and unchosen player outcomes shown), to test whether biased sampling explains the group bias effect.
- **task_paradigm:** Probabilistic reward learning
- **players:** Single agent (observer/participant), multi-target (8 avatars from 2 groups; yoked to 1 high-bias demonstrator)
- **n_players:** multi-target (3+)
- **partner_type:** unclear
- **stimuli:** Same as Study 1, with additional unchosen-player feedback in full-feedback condition
- **method:** online / behavioural
- **method_full:** behavioural / online
- **main_result:** - No Group x Feedback condition interaction (β = 0.13, Wald z = 0.38, BF(incl) = 0.07) - Group bias in partial feedback (β = 0.67, Wald z = 2.82) and full feedback (β = 0.86, Wald z = 3.69) - Demonstrator-observer correlation not modulated by condition (β = -0.16, SE = 0.21) - Overall demonstrator bias predicts observer bias (r = .17) - Rules out biased sampling account
- **effect_size:** - Group x Condition: β = 0.13, SE = 0.34, BF(incl) = 0.07 - Partial feedback group effect: β = 0.67, SE = 0.24 - Full feedback group effect: β = 0.86, SE = 0.23 - Demonstrator-observer correlation: r = .17
- **learning_from:** Other (demonstrator's biased choice actions) + world (reward outcomes from group member targets)
- **learning_about:** Same as Study 1  ### ALGORITHMIC LEVEL  Same model space. Model 6 mean AIC = 102.04.
...
- **model_mb_mf:** MF
- **model_params:** α+ mean = .31, α- mean = .27, κ mean = .15, β mean = .33  All other algorithmic fields same as Study 1.  ### IMPLEMENTATION LEVEL  N/A. **analysis_type**: N/A  ### QUALITY
...
## Quality
- **sample_size:** N = 339 (from 386; 176 control, 163 full feedback; 136 women, 193 men, 1 other, 9 unreported; M_age = 40.69, SD = 11.32; MTurk)
